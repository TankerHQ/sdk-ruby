...
def deploy() -> None:
    ...
    # Note `bundle exec rake build` does not like dirty git repos,
    # so make a commit with the new changes first
    tankerci.git.run(Path.getcwd(), "add", "--update", ".")
    tankerci.git.run(Path.getcwd(), "commit", "m", f"Bump to {version}")
